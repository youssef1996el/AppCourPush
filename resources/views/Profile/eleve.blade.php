...
>>>>>>> ec020ac3984f6725af907021cb7e80d825265099
       ...
       /*  $('.btnTime').on('click',function()
                ...
                success: function (response)
                {
                    if (response.status == 200)
                    {
                        ...
                        if (response.Data && response.Data.length > 0)
                        {
                            ...
                            $('#TableProfesseurIsActive').DataTable().clear().destroy();
                            ...
                            $.each(response.Data, function (index, value)
                            {
                                                        ...
                                                        <div class="d-flex align-items-center">
                                                            <img src="${value.image}" class="rounded-circle" style="width: 45px; height: 45px; " alt="">
                                                                ...
                                                                <p class="fw-bold mb-1 NameProfesseur" style="display:inline;">${value.name}</p>
                                                                    ...
                                                                    <p class="text-muted mb-0">${value.experience}{{-- {{$item->experince}} --}}ans expérience</p>
                                ...
                                $('#TableProfesseurIsActive').find('tbody').append(cardHTML);
                            });
        ...
        /* $(".pagination-container").on("click", ".page-link", function (e)
        {
            ...
            // Extract page number from the URL using regex
            var page = url.match(/page=(\d+)/)[1];
            isLoadPage = 0;


            // Add your logic to handle the Ajax request here
            fetchProfesseurs(page, false, false, false, false, isLoadPage);

        }); */

        //$('.pagination-container').on('click', 'a', function(e) {
            //e.preventDefault();
            //var page = $(this).text(); // Update this line to get the correct page number
            //fetchProfesseurs(page, ValueCours == '' ? false : ValueCours, ValueDay == '' ? false : ValueDay, valueHour == '' ? false : valueHour, ValueType == '' ? false : ValueType, isLoadPage);
            /* e.preventDefault();
            currentPage = $(this).attr('href').split('page=')[1];
            fetchProfesseurs(currentPage,ValueCours == '' ? false : ValueCours,ValueDay == '' ? false : ValueDay,valueHour == '' ? false : valueHour ,ValueType == '' ? false :ValueType,isLoadPage); */
       // });
